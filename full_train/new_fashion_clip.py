    ...
    def encode_images(self, images: Union[List[str], List[PIL.Image.Image]], batch_size: int):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                image_embeddings.extend(self.model.get_image_features(**batch).detach())
                pbar.update(1)
            pbar.close()
        return torch.stack(image_embeddings)
    
    def encode_vector_image(self, image):
        ...
        output_tensor = image.permute(0, 1, 2)  # Convert to [C, H, W] if not already
        output_tensor = prepare_image(output_tensor, 'cuda:0', is_tensor=True)

        features_output = self.model.get_image_features(pixel_values=output_tensor)
