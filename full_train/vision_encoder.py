...
import cv2
from new_fashion_clip import FashionCLIP
import pandas as pd
import numpy as np
...
class vision_encoder(nn.Module):
    ...
    def calc_loss_vector(self, orig_image, image):
        orig_embedding = self.encoder.encode_vector_image(orig_image)
...
if __name__ == "__main__":
    ...
    image1 = io.imread("{}265071030.jpg".format(data_root))
    pil_image1 = PIL.Image.fromarray(image1)
    image2 = io.imread("{}/image.png".format(os.path.dirname(os.getcwd())))
    pil_image2 = PIL.Image.fromarray(image2)
    results = vision_encoder.calc_loss([pil_image1], [pil_image2])
