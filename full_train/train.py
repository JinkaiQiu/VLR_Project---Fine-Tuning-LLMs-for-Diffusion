...
save_path = os.path.join(parent_directory, "saved_models")
...
for step in range(num_training_steps):
    try:
        ...
        show_image(output)
        out1 = clip_model.get_image_features(preprocess_image_tensor(output, pil=False).unsqueeze(0))
        out2 = clip_model.get_image_features(preprocess_image_tensor(pil_image, pil=True).unsqueeze(0).to("cuda:0"))
